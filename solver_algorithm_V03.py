...
class RicochetRobotsGame:
    ...
    def _find_solution(self):
        ...
        """
        import heapq  # Add this import at the top of the file if needed
        ...
        open_set = []
        counter = 0  # Unique identifier to break ties in priority queue
        
        # Store state as (f_cost, counter, move_count, positions dict, moves list)
        initial_positions = {color: robot["pos"] for color, robot in self.robots.items()}
        initial_h = self._heuristic(initial_positions, target_color, target_pos)
        
        # Entry format: (f_cost, counter, positions, moves, state_key)
        entry = (initial_h, counter, initial_positions, [], self._get_state_key(initial_positions))
        heapq.heappush(open_set, entry)
        counter += 1
        
        # Keep track of visited states and their g_costs
        visited = {}
        visited[self._get_state_key(initial_positions)] = 0
        
        # A* with a maximum depth to avoid excessive computation
        max_depth = 30  # Can be higher than BFS since we're using a heuristic
        
        while open_set:
            # Get the state with the lowest f_cost
            f_cost, _, positions, moves, state_key = heapq.heappop(open_set)
            
            # Get the g_cost (number of moves made so far)
            g_cost = len(moves)
            ...
            for color in positions:
                for direction in ["N", "S", "E", "W"]:
                    ...
                    if new_pos != temp_positions[color]:
                        ...
                        # Only consider this state if we haven't seen it or found a better path
                        if new_state_key not in visited or new_g_cost < visited[new_state_key]:
                            visited[new_state_key] = new_g_cost
                            new_moves = moves.copy()
                            new_moves.append((color, direction))
                            
                            # Calculate heuristic for the new state
                            h_cost = self._heuristic(temp_positions, target_color, target_pos)
                            f_cost = new_g_cost + h_cost
                            
                            new_entry = (f_cost, counter, temp_positions, new_moves, new_state_key)
    ...
    def _heuristic(self, positions, target_color, target_pos):  
        """
        Calculate a heuristic value for A* search.
        Lower values are better (closer to solution).
        
        Args:
            positions (dict): Dictionary of robot positions
            target_color (str): Color of the target robot
            target_pos (tuple): Position of the target
            
        Returns:
            float: Heuristic value
        """
        # Manhattan distance between target robot and target position
        robot_x, robot_y = positions[target_color]
        target_x, target_y = target_pos
        
        # Basic Manhattan distance
        manhattan_dist = abs(robot_x - target_x) + abs(robot_y - target_y)
